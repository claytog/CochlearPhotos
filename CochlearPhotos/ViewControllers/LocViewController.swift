...
class LocViewController: UIViewController {
    ...
    let centreOffset: CGFloat = 50
    let mapGapTop: CGFloat = 120
    ...
    @IBAction func didPanMapListView(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == UIGestureRecognizer.State.began {
            ...
            if locationsView.frame.origin.y < mapGapTop {
                ...
                UIView.animate(withDuration:0.6, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 1, options:[] ,
                animations: { () -> Void in
                    self.locationsView.frame.origin.y = self.mapGapBottom + self.mapGapTop
                    self.mapListHeightConstraint.constant = self.mapGapTop
                    self.locListHeightOrig = self.mapGapTop
                }, completion: nil)
            }
        }
    }
...
extension LocViewController:  UITableViewDataSource, UITableViewDelegate {
    ...
    func setupLocTableView() {
        locListHeightDef = self.view.frame.height - mapGapTop
    ...
    func setHeights(){
        ...
        mapListHeightConstraint.constant = mapViewHeight/2 - centreOffset
        mapGapBottom = mapViewHeight - mapGapTop
    }
